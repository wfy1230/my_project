...
class RTDETRDetectionModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        loss_moe = torch.tensor(0.0, device=img.device)
        # MoE auxiliary loss
        if hasattr(self, 'model'):
             for m in self.model.modules():
                 if hasattr(m, 'aux_loss'):
                     loss_moe += m.aux_loss
        loss.update({'loss_moe': loss_moe * 1.5})
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_LWGA, C2f_CSSC, C2f_CNCM, C2f_HFRB, C2f_EVA, C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, ESMoE, FSConv, LFP, C2f_MFEB, C2f_PartialNetBlock, C2f_DRG, C2f_GLGM,
                 C2f_MAC, C2f_TransMixer, C2f_DEGConv, C2f_GLSS2D, C2f_SPJFB, C2f_MSInit, C2f_PFG, C2f_SMS, C2f_SMB):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, DWRC3, C3_DWR, C2f_DWR, C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic,
                     ...
                     C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, C2f_MFEB, C2f_PartialNetBlock, C2f_DRG, C2f_GLGM, C2f_MAC,  C2f_TransMixer, C2f_DEGConv, C2f_GLSS2D, C2f_SPJFB, C2f_MSInit, C2f_PFG, C2f_SMS, C2f_SMB):
                ...
                n = 1
                   ...
                   TransformerEncoderLayer_LRSA, TransformerEncoderLayer_MALA, TransformerEncoderLayer_EGSA, TransformerEncoderLayer_SWSA, TransformerEncoderLayer_DWMMSA,
                   TransformerEncoderLayer_BinaryAttn, TransformerEncoderLayer_WCA, TransformerEncoderLayer_LCGA, TransformerEncoderLayer_CGTA):
            c2 = ch[f]
            args = [ch[f], *args]
        ...
        # --------------ASF--------------
            ...
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2]
        elif m in {MFPM, ERM, CAFM_Fusion, SFSFusion, FAAFusion}:
            c1 = [ch[x] for x in f]
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        else:
